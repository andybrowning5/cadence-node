...
var CONVERSATIONS_FILE = path.join(MEMORY_DIR, "conversations.jsonl");
var FACTS_FILE = path.join(MEMORY_DIR, "facts.json");
var DELEGATE_SOCKET = "/tmp/_primordial_delegate.sock";
...
function initMemory() {
  ...
  if (!fs2.existsSync(CONVERSATIONS_FILE)) fs2.writeFileSync(CONVERSATIONS_FILE, "");
  if (!fs2.existsSync(FACTS_FILE)) fs2.writeFileSync(FACTS_FILE, JSON.stringify({ facts: [], entities: {} }));
}
function saveTurn(threadId, userMsg, assistantMsg) {
  ...
  fs2.appendFileSync(CONVERSATIONS_FILE, JSON.stringify(entry) + "\n");
}
function loadFacts() {
  try {
    return JSON.parse(fs2.readFileSync(FACTS_FILE, "utf-8"));
  } catch {
...
function getRecentConversations(limit = 10) {
  try {
    const lines = fs2.readFileSync(CONVERSATIONS_FILE, "utf-8").trim().split("\n").filter(Boolean);
...
function getContext() {
  ...
  if (recent.length) {
    ...
    for (const c2 of recent) {
      parts.push(`[${c2.timestamp}] User: ${c2.user}`);
      parts.push(`Assistant: ${c2.assistant}`);
    }
  }
...
function searchMemory(query) {
  ...
  for (const c2 of convos) {
    if ((c2.user + c2.assistant).toLowerCase().includes(q2)) {
      results.push({ type: "conversation", timestamp: c2.timestamp, user: c2.user, assistant: c2.assistant });
    }
  }
  const facts = loadFacts();
  for (const f2 of facts.facts || []) {
    if (f2.toLowerCase().includes(q2)) results.push({ type: "fact", content: f2 });
  }
  for (const [name, info] of Object.entries(facts.entities || {})) {
    const infoStr = typeof info === "string" ? info : JSON.stringify(info);
    if ((name + infoStr).toLowerCase().includes(q2)) results.push({ type: "entity", name, info: infoStr });
  }
...
function loadOrCreateId(filename, prefix) {
  ...
  try {
    const val = fs2.readFileSync(filepath, "utf-8").trim();
    if (val) return val;
  } catch {
  }
...
async function* delegateStream(msg) {
  ...
  let done = false;
  let resolveWait;
  let waitPromise = new Promise((r2) => {
    resolveWait = r2;
  });
  sock.on("data", (chunk) => {
    buf += chunk.toString();
